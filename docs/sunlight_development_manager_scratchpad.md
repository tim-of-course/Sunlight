...
| repo-backed-v0.3-next-gap | Windows CLI thread `019f5f30-1264-7d41-9acc-94e0f2e7e8e3` | `main`, log `C:\tmp\sunlight-v03-next-gap.jsonl` | Active 2026-07-14 | Audit the complete architecture against real no-fixture behavior and implement the highest-impact remaining non-deferred production gap. | Must distinguish explicit deferrals, produce exact evidence, avoid fixture/test-count closure, implement one bounded real behavior, and leave source uncommitted for manager review. |
