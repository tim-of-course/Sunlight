...
| repo-backed-topic-intent | Windows CLI thread `019f5fba-68c6-72e1-bb0a-755e308a0f20` | `main`, log `C:\tmp\sunlight-topic-intent.jsonl` | Active 2026-07-14 | Persist validated owner, visibility, and structured acceptance criteria for real topics, with CLI/MCP parity and a minimum private-export gate. | Worker is active; review must confirm migration defaults, atomic validation, derived-record parity, status/inspect output, and conservative export behavior. |
