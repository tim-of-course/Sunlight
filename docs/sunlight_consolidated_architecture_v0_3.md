...
Canonical writers and readers that perform recovery take the repository-local `.sunlight/local/command-transaction.lock`. On Windows the lock is an OS-owned exclusive file open; on other supported hosts it is a non-blocking advisory exclusive file lock. Acquisition is immediate (`timeout_ms: 0`): contention returns stable `repository_writer_busy` rather than waiting. Recovery, canonical sequence comparison, outbox staging, canonical replacement, derived-record completion, and cleanup all occur while the lock is held. Command execution, projection materialization, content-addressed blob creation, and external Git object/ref operations remain outside the lock.
