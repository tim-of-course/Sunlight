...
- Optional Super Search validation may run fixture-backed compatibility commands after temp clone init. The harness covers compat project, compat diff, status projection, inspect projection, happy-path compat import, generated-candidate atomic failure, and fixture Git export against the temporary clone; it does not claim general real-filesystem diff/import coverage.
...
| Status projection | `sun status --projection projection_compat_agent_a_0001 --fixture basic-app --json` | Fixture | Returns candidate counts, selected safe defaults, quarantine refs, local projection refs, and fixture-backed `last_import_attempt` for `op_compat_import_auth_0001`. |
| Inspect projection | `sun inspect projection:projection_compat_agent_a_0001 --fixture basic-app --json` | Fixture | Returns `compatibility_projection`, candidate summary/detail refs, local projection refs, `last_import_attempt`, `native_operation_ids` containing `op_compat_import_auth_0001`, and `native_revision_ids` containing `rev_auth_nullability_compat_0001`. |
| Compat working tree isolation | Main Git working tree edits plus `sun compat diff` and `sun compat import --projection projection_compat_agent_a_0001 --fixture basic-app --json` | Fixture plus local Git state | Unrelated main Git working tree files do not appear in compatibility diff/import and are not treated as source truth. |
| Projection-only checkpoint/export boundary | Projection-only local edits followed by checkpoint/export validation surfaces. | Fixture plus local projection state | Projection-only edits do not enter checkpoint creation or Git export content until imported into native topic revisions. |
...
- Compatibility import captures fixture projection deltas into one normal operation transaction and one topic revision, including selected delta payloads for modified source, created source through the multi-candidate fixture, delete, rename, metadata, and rename-plus-edit cases, plus atomic failure envelopes for generated, ambiguous, conflicted, policy-blocked, secret, cache, ignored-path, missing-candidate, and no-candidate cases.
