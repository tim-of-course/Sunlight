...
- Verification: latest full pass ran after `5b1d344` and covered `3b9ee2d`. Default `scripts/smoke-suite.ps1` passed via the Windows-native fallback with 136 CLI tests, 143 core tests, validation smoke, projection strategy smoke, and MVP smoke. Optional Super Search validation passed after `e02debe`.
...
- Active slice `compat-artifact-import-provenance`: add focused fixture coverage so `sun inspect artifact` exposes compatibility import provenance links for artifacts changed by compat import.
