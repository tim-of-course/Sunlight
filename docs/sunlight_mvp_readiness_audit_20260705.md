...
The prior Phase 1 lifecycle and command-surface gaps are now closed for fixture CLI acceptance: `topic create`, `session start`, `move`, `delete`, and `metadata set` return stable JSON envelopes with topic/session IDs, session generation advancement, operation/revision IDs, precondition failure behavior, tombstone/path binding details, and metadata classification changes, and the CLI help/operator docs now describe those accepted fixture-backed commands.
...
| Phase 1: native artifact IO vertical slice | `sun init`, `topic create`, `session start`, `read`, `list`, `search`, `patch`, `write`, `move`, `delete`, `metadata set`, status, inspect, authored context, read-after-write generation, structural mutation provenance, and precondition failures are covered for fixture CLI acceptance. | `crates/sun/tests/cli_json.rs` tests for topic/session lifecycle, read/list/search, patch/write, move/delete/metadata, stale write/move preconditions, status/inspect provenance, and global help text; `crates/sunlight-core/src/artifacts.rs`; `crates/sun/src/main.rs`; `docs/sunlight_native_io_phase1_spec_v0_1.md`. Latest aggregate baseline in `docs/development_manager_scratchpad.md`: after `de5c5cb`, default `scripts/smoke-suite.ps1` passed via the Windows-native fallback with 168 CLI tests, 143 core tests, validation smoke, projection strategy smoke, and MVP smoke. | Covered for fixture CLI acceptance. Remaining polish can add deeper status/inspect round trips and refresh external validation, but the lifecycle, structural mutation, and command-help surfaces are no longer blocking Phase 1 gaps. |
...
Add a deeper status/inspect round trip for accepted Phase 1 structural mutation provenance.

Rationale:

- The command implementation, CLI JSON fixtures, global help text, and operator docs now cover `topic create`, `session start`, `move`, `delete`, and `metadata set`; repeating those surfaces would duplicate completed work.
- Status and inspect coverage already exposes provenance broadly, but a small scenario that chains move, delete, and metadata changes into explicit status/inspect assertions would tighten confidence around operator diagnostics.
- External Super Search validation is older than the Phase 1 CLI completion, but it is optional and broader than the smallest remaining acceptance gap; refresh it after the provenance round trip or use it as the next project-wide confidence pass.

Acceptance should be limited to focused status/inspect verification:

- create or extend a CLI JSON test that performs accepted structural mutations and then checks status/inspect provenance fields for the resulting operations, topic revision, session generation, tombstone/path binding, and metadata classification.
- keep the scenario fixture-backed and deterministic; do not broaden into real filesystem persistence.
- run the focused CLI test plus `git diff --check`, and schedule a full smoke-suite pass once the next small coverage slice lands.
