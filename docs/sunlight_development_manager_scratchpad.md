...
| repo-backed-windows-network-isolation | Windows Codex CLI correction thread `019f5948-a649-7dd2-a733-a22557ee6823` | `main`, logs `C:\tmp\sunlight-network-isolation-final-correction.jsonl` and `C:\tmp\sunlight-network-isolation-review-correction.jsonl` | Second review correction active 2026-07-13 | Add fail-closed per-execution Windows network denial without global firewall or host ACL side effects. | Dual modes and full suite pass. Manager hold: prove stale cleanup for a validated managed root outside the repository, and force the public Job Object setup-failure path so all three stable setup codes and atomic no-record behavior are tested before commit. |
