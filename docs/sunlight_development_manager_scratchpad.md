...
| repo-backed-windows-network-isolation | Windows Codex CLI final correction thread `019f5948-a649-7dd2-a733-a22557ee6823` | `main`, log `C:\tmp\sunlight-network-isolation-final-correction.jsonl` | Final review correction active 2026-07-13 | Add fail-closed per-execution Windows network denial without global firewall or host ACL side effects. | Dual modes, typed CLI/MCP override, real toolchains, persisted cleanup failure, and stale-profile journal now pass. Final hold fixes recovery for validated custom managed roots and restores distinct filesystem/network/Job setup error taxonomy before commit. |
