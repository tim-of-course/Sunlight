...
- Verification: latest full pass ran after `ecbe476` and covered `483fac8`. Default `scripts/smoke-suite.ps1` passed via the Windows-native fallback with 135 CLI tests, 143 core tests, validation smoke, projection strategy smoke, and MVP smoke. Focused compat-session visibility checks passed after `3b9ee2d`. Optional Super Search validation passed after `e02debe`.
...
- Active verification slice `aggregate-smoke-suite-refresh-after-compat-session-visibility`: rerun the default smoke suite after compat session status visibility.
