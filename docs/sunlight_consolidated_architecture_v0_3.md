...
Individual standalone derived JSON records use canonical JSON bytes, a flushed local-only stage, and the same OS-correct atomic replacement primitive, so a final record path is old, new, or absent rather than partially written. This path remains appropriate for local/cache records and records such as policy-validation or resolver evidence that do not accompany a canonical-state change.
...
Commands that change `records/native-state.json` and declare derived JSON mirrors use one local publication outbox under `.sunlight/local/publication-outbox/`. Before canonical commit, the command flushes every canonical derived payload and a canonical manifest containing a transaction ID, target native-state `publication_sequence` and SHA-256, and each final repository-relative path, payload SHA-256, and transaction-confined staged reference. Final paths are limited to known `.sunlight` record namespaces, may not traverse symlinks or replace native state, and must have a filename formed from the portable record ID grammar `[A-Za-z0-9][A-Za-z0-9_-]*` plus `.json`. The reserved `native-state` ID and case-insensitive Windows device basenames `CON`, `PRN`, `AUX`, `NUL`, `COM1` through `COM9`, and `LPT1` through `LPT9` are invalid. The same validation runs when a record is declared and when an outbox manifest is recovered, before staging or replay can publish final bytes; backslashes, colons, alternate-data-stream syntax, dots, spaces, and other filename characters outside the grammar are rejected.

The existing native-state journal remains the canonical commit primitive. A batch is committed exactly when native state matches both the manifest's target sequence and digest. Only after that commit are declared final records published with the Windows-correct atomic replacement primitive; a flushed completion marker precedes cleanup. Startup validates the manifest and every staged canonical payload before acting. It discards a valid uncommitted batch without publishing records, idempotently finishes a committed batch only from its declared bytes, and retains the outbox with `publication_outbox_recovery_failed` if a committed manifest or staged payload is missing, malformed, path-invalid, or digest-invalid. Recovery never reconstructs undeclared bytes.

The transaction boundary is native state plus its declared `.sunlight` JSON mirrors. Content-addressed blobs are durably created before this boundary. Managed projection directories, executed processes, and Git object/ref writes occur outside it; their canonical projection, execution, checkpoint, validation, and export-map records enter the boundary when canonical state changes. Policy-validation reports and conflict/view records produced without a canonical change remain standalone atomically published records.

Canonical writers and readers that perform recovery take the repository-local `.sunlight/local/command-transaction.lock`. On Windows the lock is an OS-owned exclusive file open; on other supported hosts it is an advisory exclusive file lock. One command shares a cancellation-aware ten-second wait budget across lock acquisitions and safe CAS retries before returning stable `repository_writer_busy`; ordinary short publication overlap therefore does not escape to callers or multiply into repeated full timeouts. MCP transport metrics report the accumulated wait as `writer_wait_ms`. Recovery, canonical sequence comparison, outbox staging, canonical replacement, derived-record completion, and cleanup all occur while the lock is held. Command execution, projection materialization, content-addressed blob creation, and external Git object/ref operations remain outside the lock.

Canonical save is compare-and-swap. A state loaded at publication sequence `N` may publish only while canonical state is still exactly `N`; success writes `N+1`. The only initialization exception is an in-memory sequence of zero with no canonical state. A missing or different canonical sequence returns stable `concurrent_state_update` with `expected_sequence` and `actual_sequence` (`missing` when absent), creates no outbox, and changes no canonical or derived record. The caller must reload and reapply or re-run its mutation. This serializes publication but does not merge independently computed whole-state snapshots, roll back already completed external execution or Git side effects, or remove unreferenced content-addressed blobs created before a stale save is detected.
